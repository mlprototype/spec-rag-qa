...
def test_http_judge_requires_an_explicit_allowed_host(tmp_path: Path) -> None:
    json_path = tmp_path / "advanced-error.json"
    markdown_path = tmp_path / "advanced-error.md"
    env = os.environ.copy()
    env.pop("AGENT_EVAL_JUDGE_API_KEY", None)
    ...
    completed = subprocess.run(
        [
            ...
            "http",
            "--judge-url",
            "https://judge.example.test/evaluate",
            "--judge-model",
            "external-model",
            "--report-json",
            str(json_path),
    ...
    assert completed.returncode == 2
    report = json.loads(json_path.read_text(encoding="utf-8"))
    assert "allowlist" in report["preflight_error"]["message"]
    assert "judge.example.test" not in report["preflight_error"]["message"]
    assert markdown_path.is_file()
